...
#define MAX_CHANNELS (400 * 3)
#define NUM_PORTS 4

#define I2C_ADDR 0x10
#define CMD_WRITE_CH1 0x21

static unsigned char buf[MAX_CHANNELS + 1];
  ...
  : ChannelOutputBase(startChannel, channelCount)
{
	...
	m_maxChannels = MAX_CHANNELS * NUM_PORTS;
}
...
int DTPiHat::Init(Json::Value config)
{
    ...
    // Open I2C bus
    if((m_i2c = open("/dev/i2c-1", O_RDWR)) < 0)
    {
    ...
    {
        LogErr(VB_CHANNELOUT, "failed to acquire bus access and/or talk to slave: %s\n", strerror(errno));
        return 0;
    }
